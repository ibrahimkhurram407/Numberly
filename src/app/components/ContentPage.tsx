...
import type { Level, MediaAsset, QuestionTemplate } from '../types';
...
export const ContentPage = () => {
  ...
  const refreshContent = async () => {
    const [assetResponse, levelResponse, approvedResponse, pendingResponse] = await Promise.all([
      fetchAssets(),
      fetchLevels(),
      fetchQuestionTemplates('approved'),
      fetchQuestionTemplates('pending'),
    ]);
    ...
    if (levelResponse.levels[0] && !selectedLevelId) {
      setSelectedLevelId(levelResponse.levels[0].id);
    }
  };
  ...
  const handleVisionSuggest = async () => {
    ...
    try {
      const response = await suggestVisionLabels({ imageData });
      setVisionLabels(response.labels);
      if (!objectName && response.labels[0]) {
        setObjectName(response.labels[0].toLowerCase());
      }
  ...
  const handleTemplateSubmit = async (event: React.FormEvent) => {
    event.preventDefault();
    try {
      const response = await createQuestionTemplate({
      ...
      setTemplateAnswer('');
      setTemplateTags('');
      setTemplateMessage('Manual template saved and approved.');
    } catch (error) {
  ...
  const handleGenerateSuggestions = async () => {
    ...
    try {
      const response = await generateQuestionTemplates({
  ...
  const handleReview = async (id: number, reviewStatus: 'approved' | 'rejected') => {
    const response = await reviewQuestionTemplate(id, reviewStatus);
    ...
    <div className="flex-1 overflow-y-auto px-4 pb-32 pt-28 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-8">
        <section className="rounded-[2rem] border-2 border-slate-100 bg-white p-8 shadow-[0_14px_40px_rgba(15,23,42,0.06)]">
          ...
          <p className="mt-3 max-w-3xl font-semibold text-slate-500">
            Create reusable image assets, author manual question templates, and review Gemini suggestions before they are used in the learner experience.
          </p>
        </section>
        ...
        <div className="grid gap-8 xl:grid-cols-[0.95fr_1.05fr]">
          <form onSubmit={handleAssetSubmit} className="rounded-[2rem] border-2 border-slate-100 bg-white p-8 shadow-[0_14px_40px_rgba(15,23,42,0.06)]">
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="flex flex-wrap gap-3">
                <button type="button" onClick={handleVisionSuggest} className="inline-flex items-center gap-2 rounded-3xl bg-amber-400 px-5 py-3 font-black text-white">
                  <Sparkles size={18} />
                  Suggest labels with Vision
                </button>
                <button type="submit" className="rounded-3xl bg-slate-900 px-5 py-3 font-black text-white">
                  Save asset
                </button>
              </div>
          ...
          <form onSubmit={handleTemplateSubmit} className="rounded-[2rem] border-2 border-slate-100 bg-white p-8 shadow-[0_14px_40px_rgba(15,23,42,0.06)]">
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              <select value={selectedLevelId} onChange={(event) => setSelectedLevelId(event.target.value)} className="rounded-3xl border-2 border-slate-100 bg-slate-50 px-4 py-4 font-bold text-slate-700 outline-none">
                {levels.map((level) => (
                  <option key={level.id} value={level.id}>{level.unit} - {level.title}</option>
                ))}
              </select>
              <select value={difficulty} onChange={(event) => setDifficulty(event.target.value as typeof difficulty)} className="rounded-3xl border-2 border-slate-100 bg-slate-50 px-4 py-4 font-bold text-slate-700 outline-none">
              ...
              <select value={selectedAssetId} onChange={(event) => setSelectedAssetId(event.target.value === '' ? '' : Number(event.target.value))} className="rounded-3xl border-2 border-slate-100 bg-slate-50 px-4 py-4 font-bold text-slate-700 outline-none md:col-span-2">
                ...
                {assets.map((asset) => (
                  <option key={asset.id} value={asset.id}>{asset.title} ({asset.objectName})</option>
                ))}
              </select>
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <button type="submit" className="rounded-3xl bg-slate-900 px-5 py-3 font-black text-white">Save manual template</button>
              <button type="button" onClick={handleGenerateSuggestions} className="inline-flex items-center gap-2 rounded-3xl bg-emerald-500 px-5 py-3 font-black text-white">
                <Sparkles size={18} />
            ...
            <div className="mt-6 space-y-4">
              {pendingTemplates.map((template) => (
                <div key={template.id} className="rounded-[1.5rem] border-2 border-slate-100 p-4">
                  ...
                  <p className="mt-2 text-sm font-semibold text-slate-500">Choices: {template.choices.join(', ')}</p>
                  <div className="mt-4 flex gap-3">
                    <button type="button" onClick={() => handleReview(template.id, 'approved')} className="inline-flex items-center gap-2 rounded-3xl bg-emerald-500 px-4 py-3 font-black text-white">
                      <Check size={16} />
                      Approve
                    </button>
                    <button type="button" onClick={() => handleReview(template.id, 'rejected')} className="inline-flex items-center gap-2 rounded-3xl bg-red-500 px-4 py-3 font-black text-white">
                      <X size={16} />
                      Reject
                    </button>
                  </div>
            ...
            <div className="mt-6 space-y-4">
              {approvedTemplates.map((template) => (
                <div key={template.id} className="rounded-[1.5rem] border-2 border-slate-100 p-4">
                  ...
                  <h3 className="mt-2 text-xl font-black text-slate-900">{template.title}</h3>
                  <p className="mt-2 font-semibold text-slate-600">{template.prompt}</p>
                  <p className="mt-2 text-sm font-semibold text-slate-500">Answer: {String(template.answer)}</p>
                  <p className="mt-1 text-sm font-semibold text-slate-500">Tags: {template.tags.join(', ') || 'None'}</p>
                </div>
              ))}
